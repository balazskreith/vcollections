 ...
 */
public class ClusteredVStorageBuilder<K, V> extends AbstractVStorageBuilder {
	...
	protected <K, V, T extends IStorage<K, V> & IAccessKeyGenerator<K>> T build(AbstractVStorageBuilder.Config config, IStorage<K, V>... storages) {
		ClusteredVStorage<K, V> result = new ClusteredVStorage<K, V>(null, config.capacity, null, storages);
		return (T) result;
	}
