...
import storages.ISetKeyGenerator;
import storages.IStorage;
import storages.sort.ISorter;
import storages.sort.arrays.Quicksorter;
...
public class VArrayList<T> implements List<T> {
	...
	public VArrayList(IStorage<Long, T> storage) {
		...
		if (!(this.storage instanceof ISetKeyGenerator)) {
			throw new RuntimeException("Storage must implement ISetKeyGenerator for " + this.getClass().getName());
		}

		((ISetKeyGenerator<Long>) this.storage).setKeyGenerator(new Supplier<Long>() {
